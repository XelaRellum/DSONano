...
{ 
  ...
  for(j=y1; j<=y2; ++j) Set_Pixel(Color);
}
/*******************************************************************************
 Erase Wave Curve
*******************************************************************************/
...
{ 
  ...
  if(Color==CURVE) y1=View_Buffer[0];
  else             y1= Ref_Buffer[0]; 

  for(i=0; i<X_SIZE; ++i){
    if(Color==CURVE) y2=View_Buffer[i];
    else             y2= Ref_Buffer[i];
    Erase_SEG(i, y1, y2);    
    y1 = y2;
  }
}
      ...
      if(((v==((MIN_Y+MAX_Y)/2+1))||(v==((MIN_Y+MAX_Y)/2-1)))&&((i-MIN_X)%5==0)){
        ...
        if(((i==((MIN_X+MAX_X)/2+1))||(i==((MIN_X+MAX_X)/2-1)))&&((v-MIN_Y)%5==0)){
          ...
          if(((v==v1)||(v==v2))&&((i-MIN_X)%5==0)){ 
            ...
            if(((i==t1)||(i==t2))&&((v-MIN_Y)%3==0)) Set_Pixel(WHITE);   
            else                                     Set_Pixel(BLACK); 
          }
        }
      }
    }
      ...
      if(((t-MIN_X)%25==0)||((j-MIN_Y)%25==0)) {      
        ...
        if(((t==((MIN_X+MAX_X)/2+1))||(t==((MIN_X+MAX_X)/2-1)))&&((j-MIN_Y)%5==0)){
          ...
          if(((j==((MIN_Y+MAX_Y)/2+1))||(j==((MIN_Y+MAX_Y)/2-1)))&&((t-MIN_X)%5==0)){
            ...
            if(((j==v1)||(j==v2))&&((t-MIN_X)%5==0)) Set_Pixel(WHITE);
            else                                     Set_Pixel(BLACK);
          }
        }
      }
...
{ 
    Point_SCR(MIN_X-1, v0-2);Set_Pixel(BLACK);
    Point_SCR(MIN_X-1, v0+2);Set_Pixel(BLACK);
    Point_SCR(MAX_X+1, v0-2);Set_Pixel(BLACK);
    Point_SCR(MAX_X+1, v0+2);Set_Pixel(BLACK);

    Point_SCR(MIN_X, v0-1);Set_Pixel(GRID);
    Point_SCR(MIN_X, v0+1);Set_Pixel(GRID);
    Point_SCR(MAX_X, v0-1);Set_Pixel(GRID);
    Point_SCR(MAX_X, v0+1);Set_Pixel(GRID);

    Point_SCR(MIN_X+1, v0);
    if(((v0-MIN_Y)%25==0)||((v0-MIN_Y)%5==0)) Set_Pixel(GRID);
    else                                      Set_Pixel(BLACK);
...
{ 
    Point_SCR(MIN_X-1, vt-2);Set_Pixel(BLACK);
    Point_SCR(MIN_X-1, vt+2);Set_Pixel(BLACK);
    Point_SCR(MAX_X+1, vt-2);Set_Pixel(BLACK);
    Point_SCR(MAX_X+1, vt+2);Set_Pixel(BLACK);

    Point_SCR(MIN_X, vt-1);Set_Pixel(GRID);
    Point_SCR(MIN_X, vt+1);Set_Pixel(GRID);
    Point_SCR(MAX_X, vt-1);Set_Pixel(GRID);
    Point_SCR(MAX_X, vt+1);Set_Pixel(GRID);

    Point_SCR(MIN_X+1, vt);
    if(((vt-MIN_Y)%25==0)||((vt-MIN_Y)%5==0)) Set_Pixel(GRID);
    else                                      Set_Pixel(BLACK);
...
{ 
    Point_SCR(MIN_X-1, vi-2);Set_Pixel(BLACK);
    Point_SCR(MIN_X-1, vi+2);Set_Pixel(BLACK);
    Point_SCR(MAX_X+1, vi-2);Set_Pixel(BLACK);
    Point_SCR(MAX_X+1, vi+2);Set_Pixel(BLACK);

    Point_SCR(MIN_X, vi-1);Set_Pixel(GRID);
    Point_SCR(MIN_X, vi+1);Set_Pixel(GRID);
    Point_SCR(MAX_X, vi-1);Set_Pixel(GRID);
    Point_SCR(MAX_X, vi+1);Set_Pixel(GRID);

    Point_SCR(MIN_X+1, vi);
    if(((vi-MIN_Y)%25==0)||((vi-MIN_Y)%5==0)) Set_Pixel(GRID);
    else                                      Set_Pixel(BLACK);
...
{ 
    Point_SCR(ti-2, MIN_Y-1);Set_Pixel(BLACK);
    Point_SCR(ti+2, MIN_Y-1);Set_Pixel(BLACK);
    Point_SCR(ti-2, MAX_Y+1);Set_Pixel(BLACK);
    Point_SCR(ti+2, MAX_Y+1);Set_Pixel(BLACK);

    Point_SCR(ti-1, MIN_Y);Set_Pixel(GRID);
    Point_SCR(ti+1, MIN_Y);Set_Pixel(GRID);
    Point_SCR(ti-1, MAX_Y);Set_Pixel(GRID);
    Point_SCR(ti+1, MAX_Y);Set_Pixel(GRID);

    Point_SCR(ti, MIN_Y+1);
    if(((ti-MIN_X)%25==0)||((ti-MIN_X)%5==0)) Set_Pixel(GRID);
    else                                      Set_Pixel(BLACK);
...
{ 
  ...
  for(i=(MIN_X-1); i<(MIN_X+2); ++i){
    ...
    for(j=(v0-(1+MIN_X-i)); j<(v0+2+MIN_X-i); ++j)  Set_Pixel(CURVE);
  }
  for(i=(MAX_X-1); i<(MAX_X+2); ++i){
    Point_SCR(i, v0-(i+1-MAX_X));
    for(j=(v0-(i-MAX_X)-2); j<(v0+(i-MAX_X+1)); ++j)  Set_Pixel(CURVE);
  }
}
...
{ 
  ...
  for(i=(MIN_X-1); i<(MIN_X+2); ++i){
    ...
    for(j=(vt-(1+MIN_X-i)); j<(vt+2+MIN_X-i); ++j)  Set_Pixel(YEL);
  }  
  for(i=(MAX_X-1); i<(MAX_X+2); ++i){
    Point_SCR(i, vt-(i+1-MAX_X));       
    for(j=(vt-(i-MAX_X)-2); j<(vt+(i-MAX_X+1)); ++j)  Set_Pixel(YEL);
  }
}
...
{ 
  ...
  for(i=(MIN_X-1); i<(MIN_X+2); ++i){
    ...
    for(j=vi-(1+MIN_X-i); j<(vi+(MIN_X-i+2)); ++j)  Set_Pixel(LINE);
  }  
  for(i=(MAX_X-1); i<(MAX_X+2); ++i){
    Point_SCR(i, vi-(i+1-MAX_X));       
    for(j=(vi-(i-MAX_X)-2); j<(vi+(i-MAX_X+1)); ++j)  Set_Pixel(LINE);
  }  
}
