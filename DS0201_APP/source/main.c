    ...
    if(Key_Buffer!=0) {
      ...
      switch (Item) {
        ...
        case MEASUR_KIND:      
          ...
          if(Key_Buffer==KEYCODE_UP){
            if(Item_Index[MEASUR_KIND]<2) Item_Index[MEASUR_KIND]++;  
            else                          Item_Index[MEASUR_KIND]=0;
          }
          if(Key_Buffer==KEYCODE_DOWN){
            if(Item_Index[MEASUR_KIND]>0) Item_Index[MEASUR_KIND]--;  
        ...
        case INPUT_ATTENUATOR:      
          ...
          if(Key_Buffer==KEYCODE_DOWN){
            ...
            else Item=OUTPUT_FREQUENCY;        
          }
          if((Key_Buffer==KEYCODE_LEFT)||(Key_Buffer==KEYCODE_RIGHT)){
            Erase_Wave(MODEL); 
            Erase_Wave(CURVE);
            Item_Index[INPUT_ATTENUATOR]=1-Item_Index[INPUT_ATTENUATOR];  //x1 / x10 probe
          }
          if((Item_Index[INPUT_ATTENUATOR]==0)&&(Item_Index[Y_SENSITIVITY]>9))  
        ...
        case Y_VERNIER_1: 
          if(Key_Buffer==KEYCODE_RIGHT) Item=Y_VERNIER_2; 
          if(Key_Buffer==KEYCODE_LEFT)  Item=TRIG_LEVEL; 
          if(Key_Buffer==KEYCODE_MANU){
            Hide_v1=1-Hide_v1;                  
            Erase_Vi(v1);
        ...
        case TRIG_LEVEL:    
          ...
          if(Key_Buffer==KEYCODE_LEFT)  Item=SYNC_MODE; 
          if(Key_Buffer==KEYCODE_MANU)  Hide_vt=1-Hide_vt;        
