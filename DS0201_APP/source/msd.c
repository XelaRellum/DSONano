...
{ u32 i=0, NbrOfBlock=0, Offset=0; u8 rvalue=MSD_RESPONSE_FAILURE;
...
{ u32 i=0, NbrOfBlock=0, Offset=0; u8 rvalue=MSD_RESPONSE_FAILURE;
  ...
  MSD_CS_LOW(); Delay_us();
  while(NbrOfBlock --){
    MSD_SendCmd (MSD_READ_SINGLE_BLOCK, ReadAddr+Offset, 0xFF);//send CMD17
  ...
  MSD_CS_HIGH(); MSD_WriteByte(DUMMY);
  return rvalue;
}
